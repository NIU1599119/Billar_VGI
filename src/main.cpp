...
int main()
{
    ...
    Rendering::Object ball9(&ball8Model);
    Rendering::Object ballS(&ball1Model);
    ...
    */
    ...
    // wall (left)
    wallAux.setPosition(glm::vec3(-(1.5-0.095), 0.76, 0.));
    ...
    while(!window.shouldClose())
    {
        ...
        if (drawTriangles)
        {
            ...
            controlledPosition = pos;   // mover camara a la posicion de la bola (se actualiza la camara en el siguiente frame asi que se vera raro)

            ball9.setPosition(pos);
            ball9.setOrientation(orient);
            ball9.setScaling(0.05715/2.0);
